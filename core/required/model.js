    ...
    class Model {
        ...
        static findByWithJoin(query, joinField, callback) {
            return new Composer(this)
                ...
                .end((err, models) => {
                    if (!err && !models.length) {
                        let err = new Error(`service.error.find_field__{name: ${this.name}`);
                        err.notFound = true;
                        return callback(err);
                    }
                    callback(err, models[0]);
        ...
        static findByMultipleJoin(query, joinFieldArray, callback) {
            return new Composer(this)
                ...
                .end((err, models) => {
                    if (!err && !models.length) {
                        let err = new Error(`service.error.relationship__{name: ${this.name}`);
                        err.notFound = true;
                        return callback(err);
                    }
                    callback(err, models[0]);
        ...
        __destroy__(callback) {
            ...
            if (!model.inStorage()) {
                setTimeout(callback.bind(model, {
                    '_query': `service.error.database_not_save`
                }, model), 1);
                return;
            }
